...
public class BinarySearchTree<T extends Comparable<T>> extends AbstractSet<T> implements CheckableSortedSet<T> {
     ...
     * Средняя
     */
    //Т=O(log n) в среднем; O(n) в худшем случае.
    //R=O(log n)
    @Override
    ...
    public Node removeRecursive(Node<T> start, T value) {
        ...
        if (comparison == 0) {
            ...
            Node<T> min = findSmallestValue(start.right);
            start = min;
            start.right = removeRecursive(right,start.value);
            start.left =left;
            return start;
        } else if (comparison < 0) {
            start.left = removeRecursive(start.left, value);
            return start;
        } else {
            start.right = removeRecursive(start.right, value);
            return start;
        }
    }
    ...
    public class BinarySearchTreeIterator implements Iterator<T> {
        ...
        @Override
        public T next() {
            if(currentIndex == values.size()) throw new NoSuchElementException();
           Node<T> node = values.pop();
           currentIndex ++;
           if(node.right != null){
               node = node.right;
               while(node != null){
